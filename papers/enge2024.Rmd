...
* Spike regressors for each high-motion outlier volume, defined as framewise displacement \textgreater 0.5 mm and flagged by *fMRIPrep* (XXX–XXX columns, mean = XXX columns).
...
There was little evidence for longitudinal change in reading skills, except for a few subtests, namely a decrease in word reading times, an increase in semantic fluency (i.e., generating words from semantic categories), an increase in verbal fluency (i.e., generating words from an initial phoneme), and a decrease in pseudoword reading times.
Mathematics skills (oral math and computation) showed a slight increase over time whereas working memory and general cognitive ability were relatively stable.
For all tests, there was large interindividual variation between children, both in their initial scores as well as in their longitudinal change over time.
...
The difference between spoken words and spoken pseudowords did not elicit any reliable BOLD activity at the beginning of the study and showed no evidence for change over the course of the study (see Figure \ref{fig:univariate-words-minus-pseudo}C and E).
...
(a) no systematic increase in reading performance based on standardized reading skill tests,
(b) limited evidence for longitudinal increases (linear or non-linear) in BOLD activity in response to spoken and written words, and
(c) no evidence for longitudinal increases in audio-visual BOLD response pattern similarity or word-related BOLD response pattern stability.
We will discuss each of these findings in turn as well as the limitations and research implications of the present study.

## No systematic change in behavioral test scores

## Limited change in BOLD activity amplitude
...
Each of these limitations will be discussed in more detail below.

## No systematic change in BOLD activity patterns
...
However, we did not observe any reliable longitudinal change (linear or quadratic) in audio-visual BOLD activity pattern similarity (see Figures \ref{fig:similarity-noise} to \ref{fig:similarity-words-minus-pseudo}).
Beyond the limitations mentioned above (and discussed in detail below), it is important to mention that our study design was not ideal for capturing similarities in BOLD activity patterns.
...
We hope that this analysis approach and our code (available at <https://github.com/SkeideLab/derivatives_template_code/blob/SLANG/univariate.py>) can be reused for future longitudinal fMRI studies or other studies with complex data acquisition schedules that would benefit from the flexibility provided by linear mixed-effects models.
