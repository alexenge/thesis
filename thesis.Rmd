...
Außerdem können wir Schriftzeichen, auf Papier gedruckt oder auf einen Bildschirm angezeigt, benutzen, um Informationen über Zeit und Raum hinweg zu übertragen, wie zum Beispiel diese Worte, die Sie gerade lesen.
...
In der ersten dieser Studien haben mir mittels fMRT untersucht, wie sich die Hirnaktivität bei der Verarbeitung geschriebener und gesprochener Worte verändert, während Kinder lesen lernen.
...
I also provide details on some of the analysis methods developed for the present studies, with hope of making them reusable for future projects, and I discuss the implications of my findings as well as limitations of the present studies and obstacles for scientific progress in this domain more broadly. 
...
In fact, the three studies are so different from one another that I cannot pretend that they were planned to build up on each other or logically follow from one another.
Instead, I have always been interested in how our brain makes sense of the raw visual input that it gets, and how in turn this understanding might feed back into how we see the world around us.
...
I will then introduce two specific examples of visual-semantic development, namely learning to understand written words and learning to understand visual objects.
...
Most crucially, modern EEG devices can sample these amplitudes at a frequency of 500--2000 Hz (i.e., one sample every 2 to 0.5 ms), which is as high or higher than the timing of the postsynaptic potentials themselves, allowing researchers to track cortical information processing in real time.
...
The BOLD effect is often interpreted as a measure of which brain areas are comparatively more or less active and how this activity co-varies with a certain experimental manipulation (e.g., which brain areas show an increase in BOLD activity when processing faces as compared to houses).
...
The spatial resolution, on the other hand, is much better than in EEG, as fMRI measures each individual voxel inside the brain and therefore does not suffer from the inverse problem.
...
In the third study included in this thesis (see Sections \ref{instant-effects-of-semantic-information-on-visual-perception} and the original research article), we tested this idea empirically by presenting human adults with previously unfamiliar objects (as in the example above) and measuring their EEG response to the object before, during, and after they learnt about the their functional meaning.
...
By doing so, a meta-analysis can (a) show which effects reported in the original papers are likely to be robust and replicable, thereby reducing the number of false positives, and (b) identify novel effects that individual original papers might not have had the statistical power to detect, thereby reducing the number of false negatives [@schmidt2015].
...
Yet, this is exactly what happened over the course of the last few millennia, rapidly picking up over the past ~500 years, and in turn fueling many other great achievements of humankind, such as science and technology.
...
We acquired functional MRI (fMRI) from two groups of children in the region of Uttar Pradesh, India, which had no access to public schooling and received approximately 1.5 years of reading instruction (intervention group) or math instruction (active control group) as part of our study.
...
Although most of our learning of the meaning of visual objects takes place during infancy (e.g., by learning to differentiate high-level visual object categories) and childhood (e.g., by learning to associate abstract visual shapes with meaning during learning to read), this capacity luckily remains available as we age---or otherwise we would not be able to understand much at all in a world where we are presented with novel gadgets and symbols almost on a daily basis.
...
The latter type of evidence seems especially informative because the high temporal resolution of the EEG (on the order of microseconds, directly capturing cortical postsynaptic potentials) allows to test how early during processing one can detect high-level linguistic or semantic influences [see @athanasopoulos2020 for an extended version of this argument].
